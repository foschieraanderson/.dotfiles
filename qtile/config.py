...
keys = [
    ...
    Key(
        [mod, "control"], "Delete",
        lazy.spawn("rofi -show powermenu -modi 'powermenu:~/.config/rofi/rofi-power-menu --choices=shutdown/reboot/suspend/hibernate/logout'"),
        desc="Run power menu"
    ),
    ## rofi-network-menu
    ...
    ## Volume settings
    Key([mod, mod1], "m", lazy.spawn("pamixer -t", desc="Switch between mute and unmute")),
    ...
    Key([mod, mod1], "minus", lazy.spawn("pamixer -d 2"), desc="Decrease the volume"),
     ...
     # Brightness settings
    Key([mod, "control"], "equal", lazy.spawn("brightnessctl set +3%"), desc="Increase the brightness"),
    Key([mod, "control"], "minus", lazy.spawn("brightnessctl set 3%-"), desc="Decrease the brightness"),
    ...
    Key([mod, "shift"], "return", lazy.layout.toggle_split(), desc="Toggle split window stack"),
...
def sep(op, fg, bg):
    flag = "" if op else ""
    return widget.TextBox(text=flag, padding=-2, fontsize=40, foreground=fg, background=bg)
...
def init_widgets_list():
    widgets_list = [
        widget.TextBox('󱘊', fontsize=22, padding=10, background=theme["green"], foreground=theme["white"]),
        sep(0, theme["green"], theme["blue"]),
        widget.GroupBox(
            fontsize=20,
            margin_y=3,
            margin_x=0,
            ...
            other_screen_border=theme["blue"],
            background=theme["blue"],
            foreground=theme["base"],
        ),
        sep(0, theme["blue"], theme["base"]),
        widget.Prompt(),
        widget.WindowName(),
        widget.Notify(),
        widget.Moc(),
        widget.TextBox("󰕾", fontsize=20, padding=8, background=theme["base"], foreground=theme["yellow"],
        ...
        widget.TextBox("󰸗", fontsize=18, padding=8, background=theme["base"], foreground=theme["peach"]),
        widget.Clock(format="%d/%m/%Y", background=theme["base"], foreground=theme["peach"]),
        widget.Sep(linewidth=0, padding=8, background=theme["base"]),
        widget.TextBox("󰅐", fontsize=18, padding=8, background=theme["base"], foreground=theme["mauve"]),
        widget.Clock(format="%H:%M", background=theme["base"], foreground=theme["mauve"]),
        widget.Sep(linewidth=0, padding=8, background=theme["base"]),
        widget.Sep(linewidth=0, padding=8, background=theme["base"]),
        widget.TextBox("󰇙", fontsize=18, padding=0, background=theme["base"], foreground=theme["surface2"]),
        widget.Sep(linewidth=0, padding=8, background=theme["base"]),
        widget.TextBox(
            "",
            background=theme["base"],
...
def init_screens():
    return [
            Screen(top=bar.Bar(widgets=init_widgets_screen1(), size=32, margin=[5, 5, 0, 5], opacity=0.9)),
            Screen(top=bar.Bar(widgets=init_widgets_screen2(), size=32, margin=[5, 5, 0, 5], opacity=0.9))
        ]
